...
'''
Tetrahedral grid than encodes vertex-wise distance function values.
'''
class VDTetraSet(TetraSet):

    def __init__(self, verts: th.Tensor, tets: th.Tensor):

        super().__init__(verts, tets)

        ''' 
        Gradient of barycentric coordinates w.r.t. position for each tet.
        '''
        self.du_dx = []
        for i in range(4):
            self.du_dx.append(self.compute_du_dx(i))    # (# tet, 3)
        self.du_dx = th.stack(self.du_dx, dim=1)        # (# tet, 4, 3)

    def tet_d_grad(self, v_d: th.Tensor):
        '''
        Compute gradient of distance function for each tet.
        '''

        tet_v_d = v_d[self.tets]     # (# tet, 4)
        tet_d_grad = th.sum(tet_v_d[:, :, None] * self.du_dx, dim=1)    # (# tet, 3)
        return tet_d_grad

    def compute_du_dx(self, u_i: int):

        '''
        Compute du_i / dx
        '''
        assert u_i in [0, 1, 2, 3], ""
        apex = self.verts[self.tets[:, u_i]]      # (# tet, 3)
        if u_i == 0:
            o_face = [1, 2, 3]
        elif u_i == 1:
            o_face = [0, 2, 3]
        ...
        o_face_verts = self.verts[self.tets[:, o_face]]       # (# tet, 3, 3)
        o_face_d0 = o_face_verts[:, 1] - o_face_verts[:, 0]     # (# tet, 3)
        o_face_d1 = o_face_verts[:, 2] - o_face_verts[:, 0]     # (# tet, 3)
        o_face_normals = th.cross(o_face_d0, o_face_d1, dim=-1)    # (# tet, 3)
        o_face_normals = th.nn.functional.normalize(o_face_normals, eps=1e-5, dim=1)    # (# tet, 3)

        # compute distance from the face to the apex;
        o_face_dist_apex = th.sum(o_face_normals * (apex - o_face_verts[:, 0]), dim=1)   # (# tet,)
        
        small_entries = th.abs(o_face_dist_apex) < 1e-5
        zero_entries = th.abs(o_face_dist_apex) == 0.0
        o_face_dist_apex[small_entries] = th.sign(o_face_dist_apex[small_entries]) * 1e-5
        o_face_dist_apex[zero_entries] = 1e-5
        
        du_dx = o_face_normals / o_face_dist_apex.unsqueeze(-1)
